...
"""
Global variables used for training and calculating
"""
unigram_frequency = {}
bigram_frequency = {}
word_count = 0
...
def train(text):
	"""
	Trains the model. Storing necessary data in global variables
	"""
	text = re.sub(r'\n', "", text)
	...
	for word in unigram_frequency:
		unigram_probabilities[word] = unigram_frequency[word]/word_count
...
def unigram_sentence_probability(sentence):
	"""
	Performs calculations on the sentence based on unigrams
	"""
	sentence = re.sub(r'\n', "", sentence)
...
def bigram_sentence_probability(sentence):
	"""
	Performs calculations on the sentence based on bigrams
	"""
	sentence = re.sub(r'\n', "", sentence)
	sentence = sentence.lower()
	sentence = re.sub(r'[\-,;:!?.’\'«»()–...&‘’“”*—]', "", sentence)
